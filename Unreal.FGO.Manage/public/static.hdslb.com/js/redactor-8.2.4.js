		...
		this.opts = $.extend({
			...
			modal_video: String() +
				...
				'<form id="redactorInsertVideoForm">' +
					'哔哩哔哩地址：<input type="text" id="bilibili_video_url" size="40" /> 宽度：<input type="text" id="bilibili_video_width" size="5" value="850" /> 高度：<input type="text" id="bilibili_video_height" size="5" value="478" /> '+
	...
	Redactor.prototype = {
		...
		showVideo: function()
		{
			...
			this.modalInit(RLANG.video, this.opts.modal_video, 600, $.proxy(function()
				{
					$("#bilibili_video_url").blur(this.insertBiLIVideo);
		...
		insertBiLIVideo:function(){
			var url = $("#bilibili_video_url").val();
			if (!url) return;
			if ((mz = url.match(/http:\/\/www\.bilibili\.tv\/video\/av([0-9]+)\/(?:index_([0-9]+)\.html)?/)) || (mz = url.match(/http:\/\/acg\.tv\/av([0-9]+)(?:,([0-9]+))?/)))
			{
				$("#redactor_insert_video_area").html('<embed height="'+$("#bilibili_video_height").val()+'" width="'+$("#bilibili_video_width").val()+'" quality="high" allowfullscreen="true" type="application/x-shockwave-flash" src="http://static.hdslb.com/miniloader.swf" flashvars="aid='+mz[1]+'&page='+(mz[2] ? mz[2] : 1)+'&autostart=false" pluginspage="http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash" style="line-height: 1.5em;"></embed>');
			}else
			{
				$("#bilibili_video_url").val("地址无法被识别");
			}
		},
		...
		_imageSet: function(json, link)
		{
			...
			if (json !== false)
			{
				...
				if (link !== true)
				{
					if ($("#upload_as_background").attr("checked"))
					{
						var curImg = new Image();
						var self = this;
						curImg.src = json.filelink;
						curImg.style.display = "none";
						curImg.onload = function(){
							html = '<span style="background:url(' + json.filelink + '); width:'+this.width+'px; height:'+this.height+'px; display:block"></span>';
	...
	Construct.prototype = {
		init: function()
		{
			if (navigator.userAgent.search("MSIE") === -1)
			{
				...
				this.$el.before($('<div style="width: 110px;"><input type="checkbox" id="upload_as_background" style="float:left" /><label for="upload_as_background" style="float:left">上传为背景图片</label></div>'));
